...
class ElggxUserpointsFunctions {
	...
	*/
	public static function elggx_userpoints_add_pending($user_guid, $points, $description, $type = null, $guid = null) {
	...
	*/
	public static function elggx_userpoints_add($user_guid, $points, $description, $type = null, $guid = null) {
		...
		if (!elgg_trigger_event_results('userpoints:add', $userpoint->type, ['entity' => $userpoint], true)) {
			$userpoint->delete();
			return false;
		}
		...
		if (elgg_get_plugin_setting('moderate', 'elggx_userpoints') && $type != 'admin') {
			...
			self::elggx_userpoints_update_user($user_guid, $points);
		}
	...
	* @return false|Userpoint return false on failure or Userpoint on success
	*/
	public static function elggx_userpoints_subtract($user_guid, $points, $description, $type = null, $guid = null) {
		...
		return self::elggx_userpoints_add($user_guid, $points, $description, $type, $guid);
	}
	...
	* @return bool
	*/
	public static function elggx_userpoints_exists($user_guid, $type, $guid) {
	...
	*/
	public static function elggx_userpoints_get($user_guid) {
	...
	* @return false|void
	*/
	public static function elggx_userpoints_delete($user_guid, $meta_guid, $type = null, $description = null, $limit = false) {
	...
	* @return false|void
	*/
	public static function elggx_userpoints_delete_by_meta_guid($guid) {
	...
	*/
	public static function elggx_userpoints_delete_by_userpoint($guid) {
		...
		// Decrement the users total points
		self::elggx_userpoints_update_user($owner_guid, -$points);
	}
	...
	* @return bool
	*/
	public static function elggx_userpoints_update_user($guid, $points) {
		$user = get_user($guid);
		$points = (int) $points;
		...
		if (is_int($user->userpoints_points)) {
			...
			$users_points = self::elggx_userpoints_get($user->guid);
			$users_approved_points = $users_points['approved'];
			$user->userpoints_points = (int) $users_approved_points;
		}

		if (!elgg_trigger_event_results('userpoints:update', 'object', ['entity' => $user], true)) {
			$user->userpoints_points = (int) $user->userpoints_points - $points;
			return false;
		}
	...
	* @param  integer  $guid The guid of the userpoint entity
	*/
	public static function elggx_userpoints_moderate($guid, $status) {
		...
		if ($status === 'approved') {
			self::elggx_userpoints_update_user($entity->owner_guid, $entity->meta_points);
		}
	}
	...
	*/
	public static function elggx_userpoints_registration_award($email) {

		elgg_call(ELGG_IGNORE_ACCESS, function () use($email) {
			$guids = self::elggx_userpoints_invite_status(null, $email);
	...
	*/
	public static function elggx_userpoints_invite_status($guid = null, $email) {
	...
	*/
	public static function elggx_userpoints_validEmail($email) {
		$isValid = true;
